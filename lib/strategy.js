...
function Strategy(options, verify) {
  ...
  options.tokenURL = options.tokenURL || 'https://api.nuapi.co/oauth/access_token';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.customHeaders = options.customHeaders || {};

  if (!options.customHeaders['User-Agent']) {
    options.customHeaders['User-Agent'] = options.userAgent || 'passport-nuwe';
  }

  OAuth2Strategy.call(this, options, verify);
  this.name = 'nuwe';
  this._userProfileURL = options.userProfileURL || 'https://api.nuapi.co/oauth/authorize.json';
